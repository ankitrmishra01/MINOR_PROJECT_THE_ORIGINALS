...
const AuthPage = () => {
  ...
  const loginWithGoogle = useGoogleLogin({
    onSuccess: async (tokenResponse) => {
      console.log("Google token:", tokenResponse); // Debugging
      setIsLoading(true);
      try {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (data.success) {
        login(data.user); // from UserContext
        navigate("/dashboard");
      } else {
  ...
  return (
    <div className="min-h-screen flex bg-gray-50 dark:bg-fintech-bg text-gray-900 dark:text-white overflow-hidden transition-colors duration-300">
      ...
      <div className="hidden lg:flex w-1/2 relative flex-col justify-between p-12 overflow-hidden bg-white/50 dark:bg-slate-900 border-r border-gray-200 dark:border-white/10">
        ...
        <Link
          to="/"
          className="relative z-10 flex items-center gap-2 text-gray-600 dark:text-white/70 hover:text-gray-900 dark:hover:text-white transition-colors w-fit group"
        >
        ...
        <div className="relative z-10 max-w-lg">
          ...
          >
            ...
            <blockquote className="text-4xl font-light leading-tight mb-6 font-serif text-gray-900 dark:text-white">
              {isLogin ? t("quote_buffett") : t("quote_einstein")}
            </blockquote>
            ...
            <form onSubmit={handleSubmit} className="space-y-6">
              ...
              <div className="flex flex-col gap-4">
                <button
                  ...
                  className="flex items-center justify-center gap-2 py-2.5 rounded-lg border border-gray-200 dark:border-white/10 hover:bg-gray-100 dark:hover:bg-white/5 transition-colors text-sm font-medium w-full text-gray-700 dark:text-white"
                >
